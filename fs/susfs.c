...
int susfs_open_redirect_spoof_do_sys_openat(struct inode *inode, char *out_redirected_name, size_t out_len) {
	...
	hash_for_each_possible_rcu(OPEN_REDIRECT_HLIST, entry, node, inode->i_ino) {
			...
			entry->target_dev == inode->i_sb->s_dev)
		{
			...
			if (strlen(entry->info.redirected_pathname) >= out_len) {
				SUSFS_LOGE("out_len not big enough\n");
				err = -ENAMETOOLONG;
				goto out_srcu_read_unlock;
			}
			strncpy(out_redirected_name, entry->info.redirected_pathname, out_len);
			err = 0;
			goto out_srcu_read_unlock;
		}
	}
	...
	struct fsnotify_mark **out)
{
	...
	/*
	 * Linux 4.14: fsnotify_init_mark(mark, group)
	 * Linux 4.9:  fsnotify_init_mark(mark, free_callback)
	 */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 10, 0)
	...
	*out = m;
	...
	/*
	 * 释放 fsnotify_init_mark() 创建的初始引用。
	 * inode/group 已经持有挂载后的引用。
	 */
	fsnotify_put_mark(m);
...
/* Deferred SUSFS maintenance after KernelSU umount. */
struct work_struct susfs_extra_works;

static void susfs_run_extra_works(struct work_struct *work)
{
	/*
	 * KernelSU 调度本 worker 时，常规卸载流程已经完成。
	 * 重新遍历 SUS_PATH 列表，为重新出现的 inode 恢复标记。
	 */
	if (!ksu_cred)
		return;
